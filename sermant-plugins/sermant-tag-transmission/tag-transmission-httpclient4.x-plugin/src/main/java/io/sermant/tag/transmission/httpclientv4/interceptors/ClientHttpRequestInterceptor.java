        ...
        if (obj instanceof HttpRequest) {
            ...
            for (Map.Entry<String, List<String>> entry : TrafficUtils.getTrafficTag().getTag().entrySet()) {
                ...
                if (CollectionUtils.isEmpty(values)) {
                    ...
                    LOGGER.log(Level.FINE, "Traffic tag {0} have been injected to restTemplate.", entry);
                    continue;
                }
                for (String value : values) {
                    headers.set(key, value);
                }
                LOGGER.log(Level.FINE, "Traffic tag {0}={1} have been injected to restTemplate.", new Object[]{key,
                        values});
            }
        }
        return context;
